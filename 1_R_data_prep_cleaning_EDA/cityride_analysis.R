...
generate_categorical_summary <- function(rides_df, drivers_df) {
  ...
  cat("RIDES DATA:\n")
  cat(strrep("-", 40), "\n")
  categorical_cols <- rides_df %>% select(where(is.character)) %>% names()
  ...
  cat("\n\nDRIVERS DATA:\n")
  cat(strrep("-", 40), "\n")
  categorical_cols <- drivers_df %>% select(where(is.character)) %>% names()
  for (col in categorical_cols) {
    cat(sprintf("\n%s:\n", col))
...
plot_temporal_analysis <- function(rides_df) {
  ...
  rides_df$Day_of_Week <- factor(rides_df$Day_of_Week, levels = dow_order)
  dow_data <- rides_df %>% count(Day_of_Week) %>% complete(Day_of_Week = dow_order, fill = list(n = 0))
  ...
  p4 <- ggplot(daily_rev, aes(x = Date, y = Revenue)) +
    ...
    annotate("text", x = min(daily_rev$Date), y = avg_rev, 
  ...
  cat(sprintf("2. Slowest day: %s with %s rides\n", 
  ...
  cat(sprintf("3. Weekend vs Weekday rides: %s vs %s\n", 
...
plot_geographic_analysis <- function(rides_df) {
  ...
  # 2. Avg Fare by City
  city_fare <- rides_df %>% group_by(City) %>% summarise(Avg_Fare = mean(Fare)) %>% arrange(Avg_Fare)
...
plot_driver_performance <- function(rides_df, drivers_df, merged_df) {
  ...
  p2 <- ggplot(drivers_df, aes(x = Age)) +
    ...
    annotate("text", x = mean(drivers_df$Age), y = Inf, 
  ...
  cat(sprintf("2. Total inactive drivers: %s\n", 
  ...
  cat(sprintf("4. Most productive driver: %d with %d rides\n", 
...
plot_financial_analysis <- function(rides_df) {
  ...
  cat(sprintf("4. Rides with promo codes: %s (%.1f%%)\n", 
  ...
  cat(sprintf("6. Average daily revenue: $%s\n", 
...
print_advanced_insights <- function(rides_df, merged_df) {
  ...
  cat(sprintf("\n6. REVENUE CONCENTRATION (80/20 RULE):\n"))
  cat(sprintf("   Top 20%% of drivers (%d drivers) generate: $%s (%.1f%%)\n", 
              top_20_pct_count,
              format(round(top_20_revenue, 2), big.mark=","),
              (top_20_revenue/total_revenue)*100))
  cat(sprintf("   Bottom 80%% of drivers (%d drivers) generate: $%s (%.1f%%)\n", 
...
print_executive_summary <- function(rides_df, drivers_df) {
  ...
  # Geographic insights
  top_city_data <- rides_df %>% count(City) %>% arrange(desc(n)) %>% slice(1)
  
  cat(sprintf("\nGEOGRAPHIC PERFORMANCE:\n"))
  cat(strrep("=", 80), "\n")
  cat(sprintf("  Top Performing City:      %s (%s rides)\n", 
  ...
  cat(sprintf("\nCUSTOMER SATISFACTION:\n"))
  cat(strrep("=", 80), "\n")
  cat(sprintf("  Ratings >=4.0:            %10s (%.1f%%)\n", 
              format(high_ratings, big.mark=","), (high_ratings/total_rides)*100))
  cat(sprintf("  Ratings <3.0:             %10s (%.1f%%)\n", 
  ...
  cat(sprintf("\nPROMOTIONAL IMPACT:\n"))
  cat(strrep("=", 80), "\n")
  cat(sprintf("  Rides with Promos:        %10s (%.1f%%)\n", 
